...
class CalendarComponent extends Component {
    ...
    public function getMonthDaysCount(&$model){
        return cal_days_in_month(0,date('n',strtotime($model->date)),date('Y',strtotime($model->date)));
    }
    public function getMonthGrid(&$model) {
        $calendar['current']['days'] = range(1, $this->getMonthDaysCount($model));
        $calendar['current']['month'] = date('n',strtotime($model->date));
        date('n',strtotime($model->date)) != 1? $prevMonth = date('n',strtotime($model->date)): $prevMonth = 12;
        ...
        if(count($calendar['current']['days'])+$offset < 35) {
            ...
            $calendar['next']['month'] = date('n',strtotime($model->date))+1;
        } else if($offset === 6) {
